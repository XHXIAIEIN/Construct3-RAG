...
The first form runs check_project.py, print_sheet.py and print_sheet.py
--outline on every folder project under --examples (the example-projects
folder of the Construct-Example-Projects clone) and on each --projects
folder, and a fixed list of lookups, and writes exit code, a hash of stdout
and of stderr and their length per run. Record the old side before a change
and the new side after it; --limit 0 lifts the scripts' output limit, so that
what they print in full can be compared with a version that had none. For an old side that was not recorded, --scripts
names the scripts folder of another checkout of the clone, such as a git
worktree of the previous commit; its own data/ is then the clone it reads,
so that it does not report itself as a copy that differs.
